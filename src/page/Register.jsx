...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const userData = { name, email, password, photoURL, role };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center pt-10 bg-[#f8fafc] dark:bg-gray-900 px-4">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold text-[#0f172a] dark:text-white mb-6 text-center">Create an Account</h2>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="text-[#0f172a] dark:text-white font-medium">Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-[#0f172a] dark:text-white font-medium">Add Profile Picture</label>
            <input
              type="file"
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 border rounded-md mt-1 bg-white dark:bg-gray-700 text-black dark:text-white"
            />
            {errors.password && <p className="text-[#dc2626] text-sm">Password must be 6 characters, one capital, one symbol</p>}
          </div>

          <div>
            <label className="text-[#0f172a] dark:text-white font-medium">Select Role</label>
            <select
          ...
          <button
            type="submit"
            className="w-full bg-[#0284c7] hover:bg-[#0369a1] text-white py-2 rounded-md"
          >
            Register
          </button>
        </form>
